        ...
        <div className="container flex">
          ...
          <div className="nav">
            <ul className={navList ? "small" : "flex"}>
              {nav.map((list, index) => (
                <li key={index}>
                  <Link to={list.path} className="flex items-center gap-x-2">
                    <div style={{ display: "flex", alignItems: "center" }}>
                      <img
                        src={Object.values(list.icon)}
                        alt="icon"
                        style={{
                          width: "24px",
                          height: "24px",
                          paddingBottom: "5px",
                          // màn hình điện thoại ẩn icon
                          className: { navList } ? "hidden" : "block",
                        }}
                      />
                      <div className="px-1 text-sm">{list.text}</div>
                    </div>
                  </Link>
                </li>
              ))}
